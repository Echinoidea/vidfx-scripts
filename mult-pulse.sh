...
for FRAME_PATH in "$FRAME_DIR"/*.png; do
    ...
    BLOOM_RADIUS=20
    BLOOM_THRESHOLD=100
    INTENSITY_SCALED=$(echo "$BLOOM_INTENSITY * $SCALE_FACTOR" | bc | awk '{printf "%.0f", $0}')
    ...
    imgfx -i "$FRAME_PATH" mult "$HEX_COLOR" > "$OUTPUT_FRAME"
    # ~/code/rust/imgfx-dev/target/release/imgfx-dev -i "$FRAME_PATH" bloom $BLOOM_SCALED $BLOOM_RADIUS 100 255 > "$OUTPUT_FRAME"
